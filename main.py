...
def callback_inline(call):
    if call.message:
        if call.data.startswith('tomorrow#'):
            ...
            markup = types.InlineKeyboardMarkup(row_width=2)
            markup.add(types.InlineKeyboardButton("🗺 Viloyatni tanlash", callback_data='regions#1'))
            img = draw_picture(post.saharlik.strftime('%H:%M'), post.iftorlik.strftime('%H:%M'),
                               post.iftorlik.strftime('%d-%B'))
            text = (
                f"<b>Sana: {post.saharlik.strftime('%d-%B')}, Haftaning {post.day.lower()} kuni, Ro'zaning {post.id}-kuni.</b>\n\n"
                f"<i>Bugungi saharlik vaqti: {post.saharlik.strftime('%H:%M')}\nIftor vaqti: {post.iftorlik.strftime('%H:%M')}</i>\n\n<b>* Bu vaqt Toshkent shahri bo'yicha amal qiladi!</b>\n")
        ...
        if call.data.startswith('regions#'):
            ...
            bot.edit_message_caption(chat_id=call.message.chat.id, message_id=call.message.message_id,
                                     caption="<b>Siz yashaydigan hududni tanlang: </b>", reply_markup=markup,
                                     parse_mode="HTML")
        if call.data.startswith('region#'):
            ...
            region_saharlik_time = datetime.datetime(year=post_saharlik_year, month=post_saharlik_month,
                                                     day=post_saharlik_day, hour=post_saharlik_hour,
                                                     minute=post_saharlik_minutes, second=0)
            ...
            region_iftorlik_time = datetime.datetime(year=post_iftorlik_year, month=post_iftorlik_month,
                                                     day=post_iftorlik_day, hour=post_iftorlik_hour,
                                                     minute=post_iftorlik_minutes, second=0)
            if region_difference > 0:
                region_iftorlik_time = region_iftorlik_time + datetime.timedelta(minutes=abs(region_difference))
            else:
                region_iftorlik_time = region_iftorlik_time - datetime.timedelta(minutes=abs(region_difference))
            img = draw_picture(region_saharlik_time.strftime('%H:%M'), region_iftorlik_time.strftime('%H:%M'),
                               time.strftime('%d-%B'))
